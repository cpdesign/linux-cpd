				...
				const char *buf, size_t count)
{
	...
	if (success == 0) {
		bmp280_write_oversampling_ratio_press(data, oversampling);
				...
				struct device_attribute *attr, char *buf)
{
	...
	status = bmp280_read_temp(data, &temperature);
	if (status < 0)
		return status;
	else
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	status = bmp280_read_press(data, &pressure, &press2);
	if (status < 0)
		return status;
	else
		return sprintf(buf, "%d\n", pressure);
...
static int bmp280_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if (i2c_smbus_read_byte_data(client, BMP280_REG_ID) != BMP280_CHIP_ID)
		return -ENODEV;
			...
			const struct i2c_device_id *id)
{
	...
	data->regmap = regmap_init_i2c(client,
	...
	if (err) {
		dev_err(&client->dev, "Couldn't register sysfs hooks\n");
		goto exit_free;
	}
	
	dev_info(&data->client->dev, "Successfully initialized bmp280!\n");
...
static const unsigned short normal_i2c[] = { BMP280_I2C_ADDRESS,
							I2C_CLIENT_END };

static int __devexit bmp280_remove(struct i2c_client *client)
{
...
static struct i2c_driver bmp280_driver = {
	...
	.id_table	= bmp280_id,
	.probe		= bmp280_probe,
	.remove		= __devexit_p(bmp280_remove),
